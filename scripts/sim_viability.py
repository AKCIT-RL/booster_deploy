...
Roda um rollout headless e mede duas coisas que não exigem o robô:
  ...
  degrau 0  orçamento de torque — o torque aplicado excede os limites derated do
            firmware (T1_23DOF_CFG.effort_limit) em algum momento? A task sobrescreve
...
(30 Hz), não por substep de física (120 Hz), então o pico medido é um LIMITE INFERIOR
...
def main() -> int:
    ...
    from tasks.mimickit_steering.controllers import (
        CONTROLLERS, apply_effort_source, apply_tn_curve, make_degraded)
    ...
    derated = np.asarray(T1_23DOF_CFG.effort_limit, dtype=np.float64)
    ...
    cmd_speed = min(max(cmd_speed, cfg.policy.tar_speed_min), cfg.policy.tar_speed_max)
    ...
    print("   (qfrc_actuator amostrado a 30 Hz => pico medido é limite INFERIOR)")
