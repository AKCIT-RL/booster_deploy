...
class MujocoController(BaseController):
    ...
    def ctrl_step(self, dof_targets: torch.Tensor):
        ...
        # Deliberate: no explicit damping term here. This is a FORK choice, not
        # inherited -- upstream uses kd = self.robot.joint_damping, in 7bb1462e as
        # before. It models the hardware path, where the motor board closes
        # kp(q*-q) + kd(0-dq) on its own and the host PD would double-count.
        #
        # Note the original justification (damping arriving from the XML, as with
        # IsaacLab's ImplicitActuator) does not hold: T1_23dof.xml declares no
        # dof_damping at all. So with kd = 0 the T1 runs UNDAMPED in sim, and
        # RobotCfg.joint_damping is exercised only on the real robot -- see
        # docs/05 pitfall 1 and the Kd section of docs/06.
        kd = np.zeros_like(kp)
        # ctrl_limit = [
