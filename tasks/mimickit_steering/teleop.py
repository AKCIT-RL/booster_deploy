...
from tasks.mimickit_steering.controllers import (  # noqa: E402
    ...
    apply_effort_source, apply_gains, apply_rate, apply_tn_curve, make_degraded)
...
def main():
    ...
    parser.add_argument("--policy-dt", type=float, default=None,
                        help="override the control period [s]; default keeps the "
                             "task's own (1/30 here, 1/50 for t1_walk). Rates are "
                             "part of the policy - see docs/12")
    parser.add_argument("--decimation", type=int, default=None,
                        help="override physics substeps per policy step; with "
                             "--policy-dt this sets the physics rate")
    parser.add_argument("--gains", default="task", choices=("task", "default"),
                        help="'task' keeps the task's PD gains; 'default' uses "
                             "robots/t1.py (the f_n=4 Hz, zeta=1.5 set)")
    ...
    if (args.checkpoint is not None):
        # Policies join a RELATIVE checkpoint_path with their own task dir
        # (locomotion.py:27), so a path typed from the repo root doubles up:
        # tasks/locomotion/ + tasks/locomotion/robots/... Absolutise it when it
        # resolves against the cwd, and otherwise pass it through so the
        # documented task-relative form still works.
        ckpt = args.checkpoint
        if (not os.path.isabs(ckpt) and os.path.exists(ckpt)):
            ckpt = os.path.abspath(ckpt)
        cfg.policy.checkpoint_path = ckpt
    ...
    print(f"[exp] pd={args.pd}  effort={args.effort}  tn={args.tn}  "
          f"degrade={args.degrade}  gains={args.gains}")
    print(f"[exp] controle {1.0 / cfg.policy_dt:.1f} Hz  "
          f"fisica {1.0 / cfg.mujoco.physics_dt:.1f} Hz  "
          f"(decimation {cfg.mujoco.decimation})")
