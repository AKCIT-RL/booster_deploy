...
def _mimickit():
    ...
    if (len(_MIMICKIT) == 0):
        try:
            ...
            # Bare "No module named 'anim'" names nothing the reader can act on.
            # The usual cause is MIMICKIT_PATH being unset in a fresh shell, where
            # the default guess is a sibling checkout that may not exist.
            import os
            from . import MIMICKIT_PATH
            raise ImportError(
                "{}.\n"
                "MimicKit is not importable. Looked in:\n"
                "    {}{}\n"
                "Set MIMICKIT_PATH to the directory that contains anim/, envs/ "
                "and util/, e.g.\n"
                "    export MIMICKIT_PATH=../dev_tsinghua/MimicKit/mimickit\n"
                "Note it is resolved against the current working directory when "
                "relative, so run from the repository root.".format(
                    e, MIMICKIT_PATH,
                    "" if os.path.isdir(MIMICKIT_PATH) else "   (does not exist)")
            ) from e
...
def assert_joint_order_matches(char_model, robot_joint_names):
    ...
    # Since upstream 7bb1462e the config uses the asset's own naming
    # (lower_snake_case with a _joint suffix), so all 23 names now match the MJCF
    # LITERALLY. Before that release the config carried the SDK's CamelCase
    # ("AAHead_yaw" vs "aahead_yaw_joint", bare "Waist" vs "waist_yaw_joint"),
    # which is why matching is by prefix after normalisation rather than by
    # equality. The loose form is kept deliberately: it still passes unchanged
    # against both naming schemes, so this task keeps working on a pre-release
    # checkout, and it costs nothing in strictness for the failure that matters.
    #
