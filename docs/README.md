...
referentes ao commit `0dba32b` (branch `tsinghua`), e foram **revistas contra o release upstream
`7bb1462e` (2026-09-13)** na branch `tsinghua-v2` — comece por
[11](11-reavaliacao-release-2026-09.md) para saber o que mudou antes de confiar em qualquer
`arquivo:linha` abaixo. Trechos derivados de strings/símbolos do
...
11. [Reavaliação: o release de 2026-09](11-reavaliacao-release-2026-09.md) — o que o release
    `7bb1462e` mudou, o veredito de cada armadilha, e a runlist reproduzida sobre a base nova.
12. [A política da Booster como controle](12-politica-vendor-t1-walk.md) — o `t1_walk` da vendor
    pelos mesmos eixos: o que ele observa, por que é imune à lacuna de observabilidade, e o que
    isso reenquadra nas Figs. 3 e 5.
...
> **Desatualizado pelo release `7bb1462e`:** o binding Python do SDK foi eliminado e o RPC virou
> um **serviço ROS 2 de verdade** (`booster_interface.srv.RpcService`, via `booster_rpc_service`).
> Ver [11 §2c](11-reavaliacao-release-2026-09.md#2c-o-binding-python-do-sdk-deixou-de-existir).
...
> **Desatualizado pelo release `7bb1462e`:** a saída não é mais fixa em `kWalking` — é
> `booster.exit_mode` (`walking` | `damping`), com `--exit-mode` na linha de comando e `damping`
> forçado em abort de segurança. Ver [06](06-runbook-deploy-real.md#como-abortar).
...
`booster_deploy/robots/t1.py` (o `robots/booster.py` virou um shim de re-export no release) e
sobrescritíveis por tarefa. Ver [03](03-ganhos-pd-sim-vs-real.md).

> **Importante no robô real:** `joint_damping` vai **direto aos motores**, e para as juntas
> paralelas do tornozelo o valor de lado motor não é o de junta. Ver
