...
>
> **Nota de versão (release `7bb1462e`).** Os ganhos do T1 deixaram de ser valores redondos
> (`80`/`2`) e passaram a ser finos (`33.09…`/`3.95…`). Não são empíricos: resolvendo
> `Kp = J_eq (2π f_n)²` e `Kd = 2 ζ J_eq (2π f_n)` com `J_eq` = `armature` do MJCF, **todas as 23
> juntas** caem em **f_n = 4,0 Hz e ζ = 1,5** — um projeto de segunda ordem uniforme, com uma
> correção paralela só nos tornozelos. A derivação e o que ela implica no deploy estão em
